...
class UserViewModel: ViewModel() {
    ...
    private val teacherApi =  RetrofitInstance.teacherApi
    ...
    private val _teacherTokenResult = MutableLiveData<NetworkResponse<tokenData>>()
    val teacherTokenResult : LiveData<NetworkResponse<tokenData>> = _teacherTokenResult
    ...
    fun GradeBookLogin(username:String , password: String){
        Log.i(TAG, "GradeBookLogin: password  $password")
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                val response = teacherApi.loginTeacher(loginRequestData(username,password))

                if (response.isSuccessful){
                    Log.i(TAG, "data successfully taken " + response.body().toString())

                    response.body()?.let {
                        _teacherTokenResult.value = Success(it)
                    }
                } else {
                    Log.e(TAG, response.code().toString())
                    _teacherTokenResult.value = Error("Failed to fetch data")
                }
            }
            catch (e: Exception){
                Log.e(TAG, e.message.toString())
                _teacherTokenResult.value = Error("Exception: ${e.message}")
            }
        }
    }
    ...
    fun resetTeacherResult(){
        _teacherTokenResult.value = Idle
    }
}